...
describe('Monads', () => {
  test.todo('Use map/flatMap to safely get the street name when optionally given a user.', () => {
    const user: User = {
      id: 2,
      name: 'albert',
      address: {
  ...
  test.todo("Use getFile to get the filename, remove the directory so it's just the file, then log it.", async () => {
    const result = await Effect.runSync(logFilename)
    expect(result).toBe('monads.ts')
  })

  test.todo("Use `getPost` then pass the post's id to `getComments`.", async () => {
  ...
  test.todo('Use `validateEmail`, `addToMailingList`, and `emailBlast` to implement `joinMailingList`.', () => {
